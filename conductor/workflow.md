...
- [ ] No linting or static analysis errors (Biome)
...
bun run test         # Run all tests (only failures by default)
```
...
### Integration Testing
- Test complete user flows in the TUI.
- Verify database state after operations.
- Test addon installation/update from mock providers.
...
### Message Format
Structure commits exactly as follows:
```
...
### Core Rules
- **Conventional Commits:** Adhere strictly to the format above.
- **Commit Frequently:** Commit changes after every task, especially for large features.
- **No AI Attribution:** Do not include "AI generated" or similar phrases in commit messages or notes.
...
### Examples
```bash
git commit -m "feat(ui): add modern gradient to header"
git commit -m "fix(tukui): correct version parsing for elvui"
git commit -m "refactor(db): use bun:sqlite native migrations"
```
