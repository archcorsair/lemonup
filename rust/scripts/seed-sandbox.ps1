...
Reset-LemonupProfileState -Profile $Profile
...
foreach ($target in $matrix) {
    ...
    $installLine = $result.Output | Where-Object { $_ -match ' install: ' } | Select-Object -Last 1
    if ([string]::IsNullOrWhiteSpace($installLine)) {
        throw "seed install for $($target.selector) did not produce an install summary"
    }
